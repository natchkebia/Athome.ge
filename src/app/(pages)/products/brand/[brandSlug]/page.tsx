...
export default function BrandProductsPage() {
  ...
  return <><Breadcrumb items={breadcrumbs} /><div className={`${styles.container} site-wrapper`}>
    <div className={`${styles.sidebar} ${styles.desktopSidebar}`}>{schema && <BrandCategoryFilter schema={schema} value={selection} onChange={value => { setSelection(value); setCurrentPage(1); }} />}</div>
    <main className={styles.content}><div className={styles.sortbarWrapper}>
      <ProductSortBar filters={{ sort }} onChange={({ sort: next }) => { if (next) { setSort(next); setCurrentPage(1); } }} />
    ...
    {mobileFiltersOpen && schema && <div className={styles.mobileFilterPanel}><BrandCategoryFilter schema={schema} value={selection} onChange={value => { setSelection(value); setCurrentPage(1); setMobileFiltersOpen(false); }} /></div>}
