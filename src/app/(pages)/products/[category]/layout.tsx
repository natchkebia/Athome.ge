...
const SITE_URL = "https://athome.ge";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (API_BASE_URL) {
    try {
      const url = new URL(`/api/storefront/categories/${encodeURIComponent(category)}`, API_BASE_URL);
  ...
  const image = data?.seo?.ogImageUrl;
  return {
    title,
    description,
...
export default function CategoryMetadataLayout({ children }: Props) { return children; }
