...
export default function BrandCategoryFilter({ schema, value, onChange }: {
  ...
  return <aside className={styles.wrapper}>
    ...
    <button className={`${styles.row} ${!value.categorySlug && !value.subCategorySlug ? styles.active : ""}`} onClick={() => onChange({})}>
      <span>{en ? "All products" : "ყველა პროდუქტი"}</span><b>{schema.totalProductCount}</b>
    </button>
    {schema.categories.map((category) => {
      ...
      return <div key={category.slug} className={styles.group}>
        ...
        {expanded && category.subCategories.map(sub => <button key={sub.slug} className={`${styles.subRow} ${value.subCategorySlug === sub.slug ? styles.active : ""}`} onClick={() => onChange({ subCategorySlug: sub.slug })}><span>{sub.name}</span><b>{sub.count}</b></button>)}
      </div>;
    })}
  </aside>;
}
